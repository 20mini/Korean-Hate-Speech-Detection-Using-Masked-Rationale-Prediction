...
def get_dataset_for_explain(data_dir, tokenizer, method):
    ...
    for div in ['train', 'val', 'test']:
        for data in dataset[div]:
            ...
            if method == 'union':
                bi_rat = data['OFF_token_labels']
                bi_rat = [int(each) for each in bi_rat]

            bi_rat_token = get_token_rationale(tokenizer, copy.deepcopy(
                text.split()), bi_rat, copy.deepcopy(post_id))
            ...
            #     continue
            output_list.append(
                [post_id, data['final_label'], encoded_text, bi_rat_token, label_list])
    return output_list
...
def convert_to_eraser_format(dataset, method, save_split, save_path, id_division):
    ...
    for tcount, eachrow in enumerate(dataset):
        ...
        temp['evidences'] = [get_evidence(
            post_id, majority_label, list(anno_text_list), final_explanation)]
...
def get_explain_results(args):
    data_dir = os.path.join(args.dir_hatexplain, 'kold.json')
    # The dataset in Eraser format will be stored here
    save_path = '/content/drive/MyDrive/4-1_자주연/mrp_hate-speech-detection/metrics/'
    if not os.path.exists(save_path):
        os.makedirs(save_path)

    # If should get dataset in Eraser format
    method = 'union'
    get_eval_data = True  # turn it to True
    if get_eval_data:
        ...
        save_split = True
        _ = convert_to_eraser_format(
            recon_data, method, save_split, save_path, id_division)
        print('done')
    ...
    score_file_dir = os.path.join(
        '/'.join(result_file_dir.split('/')[:-1]), 'explain_result.json')

    os.chdir(
        '/content/drive/MyDrive/4-1_자주연/mrp_hate-speech-detection/eraserbenchmark')
    os.system('PYTHONPATH=./:$PYTHONPATH python rationale_benchmark/metrics.py --split test --strict --data_dir {} --results {} --score_file {}'.format(
        save_path, result_file_dir, score_file_dir))
    ...
    print('* Comprehensiveness :',
          output_data['classification_scores']['comprehensiveness'])
    ...
    log.write('IOU F1: {}\n'.format(
        output_data['iou_scores'][0]['macro']['f1']))
    log.write('Token F1: {}\n'.format(
        output_data['token_prf']['instance_macro']['f1']))
    ...
    log.write('Sufficiency: {}\n'.format(
        output_data['classification_scores']['sufficiency']))

    # LIME
    result_file_dir = args.dir_result + '/for_explain_lime.json'
    score_file_dir = os.path.join(
        '/'.join(result_file_dir.split('/')[:-1]), 'explain_result_lime.json')
