...
def get_args_1():
    ...
    parser.add_argument('--pretrained_model', default='klue/bert-base',
                        choices=model_choices, help='a pre-trained bert model to use')
    ...
    # Pre-Finetuing Task
    parser.add_argument(
        '--intermediate', choices=['mrp', 'rp'], required=True, help='choice of an intermediate task')
...
def get_prob_acc(args, gts_tensor, ori_gts, out_logits):
    ...
    for gt, ori_gt, logit in zip(gts_tensor, ori_gts, out_logits):
        ...
        assert len(pred_prob) == len(
            gt_prob), "[!] get_prob_acc() | #gt != #pred"
...
def evaluate(args, model, dataloader, tokenizer, emb_layer, mlb):
    ...
    with torch.no_grad():
        for i, batch in enumerate(tqdm(dataloader, desc="EVAL | # {}".format(args.n_eval), mininterval=0.01)):
            ...
            if args.intermediate == 'rp':
                ...
                gts = prepare_gts(args, max_len, batch[2])
                masked_idxs, label_reps, masked_gts = make_masked_rationale_label(
                    args, gts, emb_layer)
                gts_pad, masked_gts_pad, label_reps = add_pads(
                    args, max_len, gts, masked_gts, label_reps)
    ...
    if args.intermediate == 'mrp':
        ...
        f1.append(f1_score(all_gts_masked_only,
                  all_pred_clses_masked, average='macro'))
...
def train(args):
    ...
    if args.intermediate == 'mrp':
        optimizer = optim.RAdam(list(emb_layer.parameters(
        ))+list(model.parameters()), lr=args.lr, betas=(0.9, 0.99))
    ...
    for epoch in range(args.epochs):
        ...
        for i, batch in enumerate(tqdm(train_dataloader, desc="TRAIN | Epoch: {}".format(epoch), mininterval=0.01)):
            ...
            if i == 0 or (i+1) % args.val_int == 0:
                _, val_loss, val_time, acc, f1 = evaluate(
                    args, model, val_dataloader, tokenizer, emb_layer, mlb)
                ...
                print(
                    "* val_loss: {} | val_consumed_time: {}".format(val_loss, val_time))
                print("* acc: {} | f1: {}".format(acc[0], f1[0]))
                if args.intermediate == 'mrp':
                    print(
                        "* acc about masked: {} | f1 about masked: {}".format(acc[1], f1[1]))
                print('\n')
                ...
                if args.intermediate == 'mrp':
                    log.write(
                        "* acc about masked: {} | f1 about masked: {}\n".format(acc[1], f1[1]))
                log.write('\n')
...
if __name__ == '__main__':
    ...
    args.exp_name = args.exp_date + '_' + lm + '_' + args.intermediate + "_" + \
        str(args.lr) + "_" + str(args.batch_size) + "_" + str(args.val_int)
