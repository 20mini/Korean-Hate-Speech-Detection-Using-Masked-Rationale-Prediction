...
class HateXplainDataset(Dataset):
    def __init__(self, args, mode='train'):
        assert mode in ['train', 'val',
                        'test'], "the mode should be [train/val/test]"

        data_root = args.dir_hatexplain
        data_dir = os.path.join(data_root, 'kold.json')
        ...
        if mode == 'train':
            ...
            for d in self.dataset:
                if d['final_label'] in self.label_list:
                    self.label_count[self.label_list.index(
                        d['final_label'])] += 1
        elif mode == 'val':
        ...
        if args.intermediate:
            ...
            for idx, d in enumerate(self.dataset):
                # 수정:d['final_rationale']
                if 1 not in d['OFF_token_labels'] and d['final_label'] in ('offensive', 'hatespeech'):
                    rm_idxs.append(idx)
    ...
    def __getitem__(self, idx):
        ...
        text = emoji.demojize(text)  # use_aliases=True 삭제
        label = self.dataset[idx]['final_label']
        if label not in self.label_list:
            # assign 'normal' label to instances with labels other than 'hatespeech' and 'normal'
            label = 'normal'
        cls_num = self.label_list.index(label)

        if self.intermediate:
            # 수정:fin_rat = self.dataset[idx]['final_rationale']
            fin_rat = self.dataset[idx]['OFF_token_labels']
            fin_rat_token = get_token_rationale(self.tokenizer, copy.deepcopy(
                text.split(' ')), copy.deepcopy(fin_rat), copy.deepcopy(id))
...
class HateXplainDatasetForBias(Dataset):
    def __init__(self, args, mode='train'):
        assert mode in ['train', 'val',
                        'test'], "mode should be [train/val/test]"
...
if __name__ == '__main__':
    def get_args_1():
        ...
        parser.add_argument('--test', action='store_true',
                            help='should be True to run test.py')
        parser.add_argument(
            '-m', '--model_path', required='--test' in sys.argv, help='the checkpoint path to test')
        ...
        # Pre-Finetuing Task
        parser.add_argument('--intermediate', default='rp', choices=[
                            'mrp', 'rp'], required=not '--test' in sys.argv, help='choice of an intermediate task')
