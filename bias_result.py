    ...
    for each_model in tqdm(bias_score_file_mapping):
        ...
        for each_method in method_list:
            for each_community in community_list:
                community_data = calculate_bias_metrics(
                    bias_test_data, each_method, each_community)
                truth_values = []
                prediction_values = []
                ...
                for each in community_data['positiveID']:
                    truth_values.append(
                        label_to_value[total_data[each]['ground_truth']])
                    prediction_values.append(convert_to_score(
                        total_data[each]['classification'], total_data[each]['classification_scores']))
                ...
                roc_output_value = roc_auc_score(
...
def get_bias_results(args, bias_score_file_mapping):
    with open(os.path.join(args.dir_hatexplain, 'kold.json'), 'r') as fp:
        data = json.load(fp)

    bias_test_data = json_normalize(data['test'])
    target_group_list = ['gender', 'None', 'politics', 'individual',
                         'others', 'race', 'religion', 'multiple_attribute']
    method_list = ['subgroup', 'bpsn', 'bnsp']

    bias_dict = get_bias_dict(bias_score_file_mapping,
                              method_list, target_group_list, bias_test_data)
...
def get_plots(bias_dict):
    ...
    for each_method in ['bnsp', 'subgroup', 'bpsn']:
        ...
        for each_model in bias_dict:
            ...
            for each_community in bias_dict[each_model][each_method]:
                tuple_community.append(
                    (each_model, each_community, bias_dict[each_model][each_method][each_community]))

        df_community_score = pd.DataFrame(tuple_community, columns=[
                                          'Model', 'Community', 'AUCROC'])
        ...
        plt.xlabel(xlabel=each_method.upper(), fontdict={
                   'fontsize': 15, 'fontweight': 'bold'}, labelpad=5)
        #plt.title(each_method.upper(), loc='left', fontdict={'fontsize': 15}, pad=15)
        # print(each_method)
        plt.savefig('/home/jiyun/hatesp_detection/vis/bias_'+each_method +
