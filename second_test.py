...
def test(args):
    ...
    losses, loss_avg, acc, per_based_scores, time_avg, bias_dict_list, explain_dict_list = evaluate(
        args, model, test_dataloader, tokenizer)

    print("Loss_avg: {} / min: {} / max: {} | Consumed_time: {}\n".format(loss_avg,
          min(losses), max(losses), time_avg))
    print("** Performance-based Scores **")
    print("Acc: {} | F1: {} | AUROC: {} \n".format(
        acc[0], per_based_scores[0], per_based_scores[1]))

    log.write("Checkpoint: {}\n".format(args.model_path))
    log.write("Loss_avg: {} / min: {} / max: {} | Consumed_time: {}\n\n".format(
        loss_avg, min(losses), max(losses), time_avg))
    ...
    if args.num_labels == 2:
        ...
        with open(args.dir_result + '/for_bias.json', 'w') as f:
            f.write('\n'.join(json.dumps(i, cls=NumpyEncoder)
                    for i in bias_dict_list))

        print("** Bias-based Scores **")
        log = open(os.path.join(args.dir_result, 'test_res_bias.txt'), 'a')
        bias_score_dict, n_targets = get_bias_results(
            args, {'MODEL': args.dir_result+'/for_bias.json'})
        ...
        for each_model in bias_score_dict:
            for each_method in bias_score_dict[each_model]:
                ...
                for each_community in bias_score_dict[each_model][each_method]:
                    temp_value.append(
                        pow(bias_score_dict[each_model][each_method][each_community], power_value))
                print(each_model, each_method, pow(
                    np.sum(temp_value)/n_targets, 1/power_value))
                log.write("* {}: {}\n".format(each_method,
                          pow(np.sum(temp_value)/n_targets, 1/power_value)))
        log.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument('-m', '--model_path', type=str,
                        required=True, help='the checkpoint path to test')
    ...
    if 'ncls2' in args.model_path:
        ...
        print(
            "[!] should check num_labels your checkpoint was trained for and set the right value")
        exit()
