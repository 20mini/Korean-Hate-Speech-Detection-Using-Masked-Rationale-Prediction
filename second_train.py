...
def get_args_2():
    ...
    # DATASET
    parser.add_argument('--dir_hatexplain', type=str,
                        default="./dataset", help='the root directiory of the dataset')

    # PRETRAINED MODEL
    model_choices = ['klue/bert-base']
    parser.add_argument('--pretrained_model', default='klue/bert-base',
                        choices=model_choices, help='a pretrained bert model to use')
    ...
    # TRAIN
    parser.add_argument('--num_labels', choices=['2', '3'], default='2', required=True,
                        help="3 = [hatespeech/offensive/normal], 2 = [toxic/nontoxic]")
    ...
    parser.add_argument(
        '--top_k', default=5, help='the top num of attention values to evaluate on explainable metrics')
    parser.add_argument('--lime_n_sample', default=100,
                        help='the num of samples for lime explainer')
...
def load_model_train(args):
    ...
    model = BertForSequenceClassification.from_pretrained(
        args.pretrained_model, num_labels=args.num_labels)

    if 'mlm' in args.pre_finetuned_model:
        pre_finetuned_model = BertForMaskedLM.from_pretrained(
            args.pre_finetuned_model)
    else:
        pre_finetuned_model = BertForTokenClassification.from_pretrained(
            args.pre_finetuned_model)
...
def get_dict_for_bias(post_id, label, pred_cls, pred_prob):
    ...
    bias_dict['classification_scores'] = {
        'non-toxic': pred_prob[0], 'toxic': pred_prob[1]}
...
def get_dict_for_explain(args, model, tokenizer, in_tensor, gts_tensor, attns, id, pred_cls, pred_prob):
    ...
    explain_dict["classification_scores"] = {
        "hatespeech": pred_prob[0], "normal": pred_prob[1], "offensive": pred_prob[2]}

    attns = np.mean(attns[:, :, 0, :].detach(
    ).cpu().numpy(), axis=1).tolist()[0]
    # including start/end token ?
    top_indices = sorted(
        range(len(attns)), key=lambda i: attns[i])[-args.top_k:]
    temp_hard_rationale = []
    ...
    prob_suf = F.softmax(out_tensor_suf.logits,
    ...
    prob_com = F.softmax(out_tensor_com.logits,
                         dim=1).detach().cpu().tolist()[0]

    explain_dict['sufficiency_classification_scores'] = {
        "hatespeech": prob_suf[0], "normal": prob_suf[1], "offensive": prob_suf[2]}
    explain_dict['comprehensiveness_classification_scores'] = {
        "hatespeech": prob_com[0], "normal": prob_com[1], "offensive": prob_com[2]}
    ...
    with torch.no_grad():
        for i, batch in enumerate(tqdm(dataloader, desc="EVAL | # {}".format(args.n_eval), mininterval=0.01)):
            ...
            if args.num_labels == 2 and args.test:
                bias_dict = get_dict_for_bias(
                    ids[0], labels_list[0], pred_clses[0], probs[0])
                bias_dict_list.append(bias_dict)

            if args.num_labels == 3 and args.test:
                if labels_list[0] == 1:  # if label is 'normal'
                    continue
                explain_dict = get_dict_for_explain(
                    args, model, tokenizer, in_tensor, gts_tensor, attns, ids[0], label_dict[pred_clses[0]], probs[0])
...
def train(args):
    ...
    for epoch in range(args.epochs):
        # data: (post_words, target_rat, post_id)
        for i, batch in enumerate(tqdm(train_dataloader, desc="TRAIN | Epoch: {}".format(epoch), mininterval=0.01)):
            ...
            if i == 0 or (i+1) % args.val_int == 0:
                _, loss_avg, acc_avg, per_based_scores, time_avg, _, _ = evaluate(
                ...
                print(
                    "* val_loss: {} | val_consumed_time: {}".format(loss_avg[0], time_avg))
                print("* acc: {} | f1: {} | AUROC: {}\n".format(
                    acc_avg[0], per_based_scores[0], per_based_scores[1]))

                log.write("[Epoch {} | Val #{}]\n".format(epoch, args.n_eval))
                log.write("* tr_loss: {}\n".format(tr_loss))
                log.write(
                    "* val_loss: {} | val_consumed_time: {}\n".format(loss_avg[0], time_avg))
...
if __name__ == '__main__':
    ...
    # ex) 'bert-base' - check 'first_train.py'
    ...
    args.exp_name = args.exp_date + "_" + lm + "_" + 'ncls' + \
        str(args.num_labels) + "_" + str(args.lr) + "_" + \
        str(args.batch_size) + "_" + str(args.val_int)
    folder_name = '.'.join(
        args.pre_finetuned_model.split('/')[-1].split('.')[:-1])
