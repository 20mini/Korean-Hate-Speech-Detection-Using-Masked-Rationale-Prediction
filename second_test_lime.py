...
class TestLime():
    def __init__(self, args):
        ...
        self.explainer = LimeTextExplainer(class_names=[
                                           'hatespeech', 'normal', 'offensive'], split_expression='\s+', random_state=333, bow=False)
    ...
    def test(self, args):
        ...
        for i, batch in enumerate(tqdm(self.dataloader, desc="EVAL | # {}".format(args.n_eval), mininterval=0.01)):
            ...
            exp = self.explainer.explain_instance(
                texts[0], self.get_prob, num_features=6, top_labels=3, num_samples=args.lime_n_sample)
            ...
            temp["classification_scores"] = {"hatespeech": exp.predict_proba[0],
                                             "normal": exp.predict_proba[1], "offensive": exp.predict_proba[2]}
            ...
            for i in range(len(tokens)):
                temp_tokens = self.tokenizer.encode(
                    tokens[i], add_special_tokens=False)
            ...
            topk_indices = sorted(range(len(attention)),
                                  key=lambda i: attention[i])[-args.top_k:]
            ...
            temp["sufficiency_classification_scores"] = {
                "hatespeech": suf_probs[0][0], "normal": suf_probs[0][1], "offensive": suf_probs[0][2]}
            temp["comprehensiveness_classification_scores"] = {
                "hatespeech": com_probs[0][0], "normal": com_probs[0][1], "offensive": com_probs[0][2]}
